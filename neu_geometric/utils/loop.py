...
def add_remaining_self_loops(edge_index, edge_weight=None, fill_value=1,
    ...
    """
    num_nodes = maybe_num_nodes(edge_index, num_nodes)
    row, col = edge_index  # 分别将edge_index的第一维赋值给row，第二维赋值给col

    mask = row != col  # bool列表, 表示非self-loop的边
    # 添加自环的本质是增加边，所以相应的edge_weight和edge_index都要增长
    if edge_weight is not None:
        assert edge_weight.numel() == edge_index.size(1)
        inv_mask = ~mask  # bool列表，表示在所有边中为self-loop的
        # 注意python中定义单元素元组的“陷阱”，(1, )
        loop_weight = torch.full(
            (num_nodes, ), fill_value,
            dtype=None if edge_weight is None else edge_weight.dtype,
            device=edge_index.device)
        # remaining_edge_weight是除去那些已经具有自环
        remaining_edge_weight = edge_weight[inv_mask]
        if remaining_edge_weight.numel() > 0:
            # 对于有self_loop的节点，其row和col的值是一样的
            # 不需要重复增加的self_loop的边，其特征保持不变
            loop_weight[row[inv_mask]] = remaining_edge_weight
        # 把具有self_loop的节点的索引排在边的特征矩阵的最后
        edge_weight = torch.cat([edge_weight[mask], loop_weight], dim=0)
    # arange生成一个从0到num_nodes的等差数列
    loop_index = torch.arange(0, num_nodes, dtype=row.dtype, device=row.device)
    # unsqueeze在第0维之前添加一个维度, repeat(2,1)表示第0维复制1次，第1维不变
    loop_index = loop_index.unsqueeze(0).repeat(2, 1)
    # cat表示拼接矩阵
    # 将具有self_loop的节点的索引排在边index的最后，一定和edge_weight的顺序保持一致
